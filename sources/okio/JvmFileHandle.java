...
public final class JvmFileHandle extends FileHandle {
    ...
    protected synchronized int protectedRead(long fileOffset, byte[] array, int arrayOffset, int byteCount) {
        ...
        while (true) {
            ...
            int read = this.randomAccessFile.read(array, arrayOffset, byteCount - i11);
            if (read != -1) {
                i11 += read;
            } else if (i11 == 0) {
                return -1;
    ...
    protected synchronized void protectedResize(long size) {
        try {
            ...
            if (j11 > 0) {
                int i11 = (int) j11;
                protectedWrite(size2, new byte[i11], 0, i11);
            } else {
