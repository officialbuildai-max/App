...
class FindFileFromCache implements IDownloadTask {
    ...
    private boolean checkDownloadReadyRate(long j11) {
        return Utils.getDownloadRate(this._downloadModel.getTotalBytes(), j11) >= this._downloadMessage.getDownloadRate();
    }
    ...
    private void resumeDownloadFromFile(DownloadResponse downloadResponse, long j11) {
        this._request.setTotalBytes(this._downloadModel.getTotalBytes());
        this._request.setDownloadedBytes(j11);
        GlobalComponent.getInstance().getDatabaseHelper().update(DownloadModel.create(this._downloadModel.getDownloadId(), this._downloadModel.getDownloadUrl(), this._downloadModel.getResourceUrl(), this._downloadModel.getEtag(), this._downloadModel.getSaveDirectorPath(), this._downloadModel.getSaveFileName(), this._downloadModel.getTotalBytes(), j11, this._downloadModel.getDownloadRate(), this._downloadModel.getUsageCounter(), this._downloadModel.getDownloadResourceType()), this._request.getCacheDirectoryPath());
        downloadResponse.setSuccessful(checkDownloadReadyRate(j11));
    }
