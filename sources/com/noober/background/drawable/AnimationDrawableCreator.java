...
public class AnimationDrawableCreator implements ICreateDrawable {
    ...
    private void addFrame(int i11, int i12) {
        Drawable drawable;
        if (!this.animationTa.hasValue(i11) || (drawable = this.animationTa.getDrawable(i11)) == null) {
            return;
        }
        if (this.animationTa.hasValue(i12)) {
            this.drawable.addFrame(drawable, this.animationTa.getInt(i12, 0));
        } else {
    ...
    public Drawable create() throws Exception {
        Drawable drawable;
        for (int i11 = 0; i11 < this.animationTa.getIndexCount(); i11++) {
            int index = this.animationTa.getIndex(i11);
