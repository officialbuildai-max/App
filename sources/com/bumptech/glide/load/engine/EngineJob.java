...
class EngineJob<R> implements DecodeJob.Callback<R>, FactoryPools.Poolable {
    ...
    public class CallLoadFailed implements Runnable {
        ...
        private final ResourceCallback f20227cb;

        CallLoadFailed(ResourceCallback resourceCallback) {
            this.f20227cb = resourceCallback;
        }

        @Override // java.lang.Runnable
        public void run() {
            synchronized (this.f20227cb.getLock()) {
                synchronized (EngineJob.this) {
                    try {
                        if (EngineJob.this.cbs.contains(this.f20227cb)) {
                            EngineJob.this.callCallbackOnLoadFailed(this.f20227cb);
                        }
    ...
    public class CallResourceReady implements Runnable {
        ...
        /* renamed from: cb, reason: collision with root package name */
        private final ResourceCallback f20228cb;

        CallResourceReady(ResourceCallback resourceCallback) {
            this.f20228cb = resourceCallback;
        }

        @Override // java.lang.Runnable
        public void run() {
            synchronized (this.f20228cb.getLock()) {
                synchronized (EngineJob.this) {
                    try {
                        if (EngineJob.this.cbs.contains(this.f20228cb)) {
                            EngineJob.this.engineResource.acquire();
                            EngineJob.this.callCallbackOnResourceReady(this.f20228cb);
                            EngineJob.this.removeCallback(this.f20228cb);
                        }
                        EngineJob.this.decrementPendingCallbacks();
                    } catch (Throwable th2) {
                        throw th2;
    ...
    public static final class ResourceCallbackAndExecutor {
        ...
        /* renamed from: cb, reason: collision with root package name */
        final ResourceCallback f20229cb;
        final Executor executor;

        ResourceCallbackAndExecutor(ResourceCallback resourceCallback, Executor executor) {
            this.f20229cb = resourceCallback;
            this.executor = executor;
        }

        public boolean equals(Object obj) {
            if (obj instanceof ResourceCallbackAndExecutor) {
                return this.f20229cb.equals(((ResourceCallbackAndExecutor) obj).f20229cb);
            }
            return false;
        }

        public int hashCode() {
            return this.f20229cb.hashCode();
        }
    }
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public EngineJob(GlideExecutor glideExecutor, GlideExecutor glideExecutor2, GlideExecutor glideExecutor3, GlideExecutor glideExecutor4, EngineJobListener engineJobListener, EngineResource.ResourceListener resourceListener, f fVar) {
        ...
        synchronized (this) {
            try {
                ...
                while (it.hasNext()) {
                    ...
                    next.executor.execute(new CallLoadFailed(next.f20229cb));
                }
    ...
    void notifyCallbacksOfResult() {
        synchronized (this) {
            try {
                ...
                while (it.hasNext()) {
                    ResourceCallbackAndExecutor next = it.next();
                    next.executor.execute(new CallResourceReady(next.f20229cb));
                }
                decrementPendingCallbacks();
            } catch (Throwable th2) {
                throw th2;
