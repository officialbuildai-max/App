...
public class MultiModelLoader<Model, Data> implements ModelLoader<Model, Data> {
    private final f exceptionListPool;
    ...
    static class MultiFetcher<Data> implements DataFetcher<Data>, DataFetcher.DataCallback<Data> {
        ...
        private final f throwableListPool;

        MultiFetcher(@NonNull List<DataFetcher<Data>> list, @NonNull f fVar) {
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public MultiModelLoader(@NonNull List<ModelLoader<Model, Data>> list, @NonNull f fVar) {
        this.modelLoaders = list;
        this.exceptionListPool = fVar;
    }

    @Override // com.bumptech.glide.load.model.ModelLoader
    public ModelLoader.LoadData<Data> buildLoadData(@NonNull Model model, int i11, int i12, @NonNull Options options) {
        ModelLoader.LoadData<Data> buildLoadData;
        int size = this.modelLoaders.size();
        ArrayList arrayList = new ArrayList(size);
        Key key = null;
        for (int i13 = 0; i13 < size; i13++) {
            ModelLoader<Model, Data> modelLoader = this.modelLoaders.get(i13);
